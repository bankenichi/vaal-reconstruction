...
2b. **Confidence taxonomy and hardening.** Tiers are: **H** hardened (top), **C** committed but latitude-dependent, **S** soft candidate (appendix §9; statistically close to noise, not evidence on its own), **O** opaque; **L** lore-anchored is an orthogonal tag (H+L, C+L), and a star (C\*) flags a logged adversarial competitor. A token earns **H** only by passing BOTH gates of the lexicon hardening protocol (`analysis/HARDENING_PROTOCOL.md`): (1) it reconstructs under the strict latitude, and (2) it survives a blind adversarial pass with no genuine cross-language competitor. No token may carry `H` without both gates recorded. Apply this test to every token going forward, and cite the protocol in the token's entry. The empirical basis (why these gates matter) is master §17 and `analysis/NULL_MODEL_RESULTS.md`.
...
- Statistical hardening complete (§17): headline base 65 committed tokens, 25 hardened (Battery E blind-certified the three post-battery names; the original 62 / 22 run, Batteries A-D, is the prior epoch); null model rejected; distinct-root recount in §17.7. Companion detail in `analysis/STATISTICAL_SUMMARY.md`.
- Syntax and grammar (§3): derived in isolation from the corpus, blind-confirmed by 3 fresh analysts (18/18) with a scrambled-corpus negative control (§17.8). Protocol/log: `SYNTAX_CONFIRMATION_PROTOCOL.md`, `SYNTAX_EXPERIMENT_LOG.md`.
