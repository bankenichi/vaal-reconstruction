...
**Status of this file.** Headlines below are from the 2026-09-16 re-run (`EXPERIMENT_RERUN_PROTOCOL.md`): de-duplicated null, form-first decode over attached text dictionaries, recovery Gate 1, same-language Gate 2. The 2026-07 batteries and the 2026-08/09 audit rescore remain the **historical / STOPGAP** record; they are not mixed into these counts. Syntax is the gloss-blind analyst-level panel (`SYNTAX_EXPERIMENT_LOG_RERUN.md`): 3/3 vs 0/2, Fisher p = 0.10, not significant; possession not locked. The 2026-07 3/3 vs 0/2 p = 0.10 is historical / not gloss-blind. Line-level translation, LOTO, and secure-line grades are companions, not PPV. Model-based PPV and sentence-level percentages stay **withdrawn as translation confidence**.

Proportions carry Wilson 95% confidence intervals. Underlying data: `token_classification.csv`, `gate1_rescore_rerun.csv`, `gate2_rescore_rerun.csv`, `NULL_MODEL_RESULTS_RERUN.md`, `BATTERY_E_RERUN.md`, `EXPERIMENT_LOG.md`, `SYNTAX_EXPERIMENT_LOG_RERUN.md`, `TRANSLATION_BATTERY_RESULTS.md`, `SECURE_LINE_TAGS.md`, `LEAVE_ONE_TEXT_OUT.md`. Archive counterparts (`gate1_rescore.csv`, `NULL_HONESTY_OUTPUT.md`, `RESCORE_OUTPUT.md`, `SYNTAX_EXPERIMENT_LOG.md`) are labelled stopgap / historical where quoted.

**Headline (re-run, 2026-09-16, Ixchel Gate 1 honesty).** Form-first lookup recovers the predeclared root, language class, and sense on **6 of 78** section-9 rows. After Gate 2 and the hardening protocol, **2 of 78 are H (2.6%, 95% CI [0.7, 8.9])** per token, or **2 of 69 distinct roots (2.9% [0.8, 10.0])**. Hardened set: *xefe, Quecholli*. *Ixchel* is C+L: Battery E recovered the Cordemex herb string, not the approved theonym / Godstealer sense (Gate 1 recovery miss; gloss unchanged). Selection-matched Fisher on the new 2x2 (Battery A C, then Battery D C) is **p = 0.09494** (p ≈ 0.095) at trial level and **p = 0.3069** (p ≈ 0.31) at item level (any-C). Recovery-scored matched tests are **p = 1** (trial) and **p = 0.9991** (item): almost no real-plant recoveries versus the pseudo D-C arm. Distinct-string count equals trial count (500/500). No unconditional binomial versus a 4.6% floor. No sigma label. Gloss-blind syntax: 3/3 vs 0/2, p = 0.10, not significant; possession not locked. Translation batteries vs gold: match+partial about 29% both arms (T-M 29.4% [24.2, 35.3]; T-D 28.2% [23.1, 34.1]); methodology does not lift agreement; it cuts clash and raises abstain. Secure-line: 0/51 secure, 12/51 working (~24%), 38/51 fragile (~75%). LOTO: 12/51 ok (~24%), 33/51 ok+partial (~65%); T3 0/13 ok. Transcriptions and corpus documentation are stable; interpretive and translation confidence is thin. Companions: `NULL_MODEL_RESULTS_RERUN.md`, `SYNTAX_EXPERIMENT_LOG_RERUN.md`, `TRANSLATION_BATTERY_RESULTS.md`, `RIVAL_READING_TOURNAMENTS.md`, `PARTICLE_PROBES_3_7.md`, `LEAVE_ONE_TEXT_OUT.md`, `SECURE_LINE_TAGS.md`, `CONSISTENCY_CORRECTIONS_2026-09-16.md`.
