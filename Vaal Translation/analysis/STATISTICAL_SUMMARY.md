...
**Headline.** Texts and transcriptions are stable. Interpretive confidence is thin. Form-first lookup recovers this project's declared root, language, and sense on **6 of 78** lexicon rows. After a rival-meaning check, **2 of 78 are hardened (2.6%, 95% CI [0.7, 8.9])**, or **2 of 69 distinct roots (2.9% [0.8, 10.0])**. Hardened set: *xefe, Quecholli*. *Ixchel* is C+L: lookup found the Cordemex herb string, not the approved Godstealer sense (gloss unchanged). Chance tests do not reject noise at 0.05 once the declared sense is required (any-hit Fisher p = 0.09494 trial / 0.3069 item; declared-sense p = 1 / 0.9991). An older all-noise rejection of about 2 x 10^-15 is withdrawn. Grammar without English: 3/3 vs 0/2, p = 0.10, not significant; possession not locked. Independent translations match or partly match the published English about 29% of the time (method-on 29.4% [24.2, 35.3]; dictionaries-only 28.2% [23.1, 34.1]). Secure-line: 0/51 secure, 12/51 working (~24%), 38/51 fragile (~75%). Leave-one-text-out: 12/51 ok (~24%), 33/51 ok or partial (~65%); Text 3 0/13 ok. Model-based "probability this gloss is right" tables and per-line confidence bands are **withdrawn; not for interpretation**.
