...
- Null model: the 2 x 10^-15 (and 10^-18 / 10^-19) all-noise headlines are withdrawn as headlines, as is the sigma column. Re-run selection-matched any-C: **p = 0.09494** trial, **p = 0.3069** item. Recovery-scored matched: **p = 1** / **p = 0.9991**. That is not a claim that the corpus is very unlikely to be pure noise. Pseudo corpus this epoch: 500 distinct of 500. Cite `AUDIT_REVIEW_2026-08-18.md`.
- PPV and sentence-level percentages: structure restored in §17.5 and `STATISTICAL_SUMMARY.md` §§3-6; labelled **withdrawn as translation confidence**. Independence products are not a Frechet floor. *yan* is not a lexicon row. The committed lexicon is the project's chosen readings; most rows are C or C*, not a probable-as-a-whole certification.
...
- Secure-line grades (2026-09-16): 0/51 secure, 12/51 working, 38/51 fragile, 1/51 opaque-blocked (`analysis/SECURE_LINE_TAGS.md`). Not PPV. Working count unchanged by Ixchel (the name is not in the 51 lines).
