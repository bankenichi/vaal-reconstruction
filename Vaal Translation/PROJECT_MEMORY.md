...
## Current state (2026-09 audit remediation)

- Master: 18 sections, roughly 147 morpheme-index elements, 59 citations. Parent copy kept byte-identical with the canonical.
- Lexicon population: **78 section-9 rows**, each with H/C/S/O; L is a tag only. `token_classification.csv` has 78 rows. Battery-tested Gate 1 is the **61-row** strict CSV. *pul / puul* has no decode artifact; its published Gate-1 pass is dropped (now C).
- Hardened after recovery scoring and protocol demotion: **17 of 78 = 21.8% [14.1, 32.2]** (14 of 61 battery-tested, plus three Battery E names). Old 22/62 and 25/65 figures withdrawn. Demoted from H: *akal, ek, tul, uch', xi, k'ux, ma, pul*.
...
- Null model: the 2 x 10^-15 (and 10^-18 / 10^-19) all-noise headlines are withdrawn, as is the sigma column (those labels were count z-scores). Honest selection-matched range: **p ≈ 0.005 to 0.06 (marginal)**. Pseudo corpus: 426 distinct of 500; some noise hits are palette substrings. Cite `AUDIT_REVIEW_2026-08-18.md`.
- PPV and sentence-level percentages: not used as translation confidence. Independence products withdrawn. *yan* is not a lexicon row.
- Syntax: report analyst-level Fisher **3/3 vs 0/2, p = 0.10**, not the rule-level 12/12 vs 0/8. Analysts were not blind to project glosses. Not a significance claim.
...
- Full-pipeline null (commit then strict-gate on each unique pseudo string) not re-run; matched A-then-D is the archived-file correction.
