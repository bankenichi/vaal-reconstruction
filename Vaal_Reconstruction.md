...
**Project status: texts stable; inferential layer is the 2026-09-16 re-run, with syntax still pending.** With all five known texts documented, the lexicon retiered (78 section-9 rows, 3 hardened after recovery scoring and Gate 2), and the syntax derived, the reconstruction has reached the limit of what the current corpus can support **on the texts**. The 10^-15 headline stays withdrawn. The 2026-07 batteries remain the historical record; the stopgap is how those files must be read; the re-run files are how current H/C counts are read. Remaining analytical debt is the gloss-blind syntax panel (`SYNTAX_EXPERIMENT_LOG_RERUN.md`) plus more Vaal text. New material would enlarge the sample, supply minimal pairs for open tokens, and test the §3 grammar against unseen clauses. The document is best read as stable on the texts, re-run-corrected on the lexicon batteries, and open on syntax significance. The tooling (`HARDENING_PROTOCOL.md`, `score.py --rerun`, `run_gates_rerun.py`) is in place so new tokens can be scored without reopening the F1/N3 holes.
...
**Re-run versus stopgap.** The 2026-09-16 files (`NULL_MODEL_RESULTS_RERUN.md`, `gate1_rescore_rerun.csv`, `gate2_rescore_rerun.csv`, `BATTERY_E_RERUN.md`) are the current lexicon and null numbers in this section. The 2026-08/09 audit and recovery rescore (`AUDIT_REVIEW_2026-08-18.md`, `RESCORE_OUTPUT.md`, `NULL_HONESTY_OUTPUT.md`) remain the **STOPGAP** reading of the 2026-07 archives; those files were not overwritten. Syntax is still pending (`SYNTAX_EXPERIMENT_LOG_RERUN.md`). Dual-base tables (per token and per distinct root) stay on the page. Do not mix epochs.
