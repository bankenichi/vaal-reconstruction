...
**Statistical standing (full detail in §17).** The 2026-09-16 experimental pass is on disk (`analysis/EXPERIMENT_RERUN_PROTOCOL.md`): recovery Gate 1, same-language Gate 2, de-duplicated null, gloss-blind syntax, T-M / T-D translation batteries, rival-reading tournaments, §3.7 particle probes, leave-one-text-out, and secure-line tags. The 2026-08/09 audit rescore remains the **historical STOPGAP** for the 2026-07 files. The published all-noise rejection of about 2 x 10^-15 (and the enlarged-base 3 x 10^-18), including the sigma column, and the older "probable lexicon" / "very unlikely pure noise" wording, are **withdrawn as headlines**. After the re-run and Ixchel Gate 1 honesty, **2 of 78 section-9 rows are hardened (2.6%, 95% CI 0.7-8.9%)** per token, or **2.9% [0.8, 10.0]** counted over 69 distinct roots (1 of 61 = 1.6% [0.3, 8.7] / 1 of 52 = 1.9% [0.3, 10.1] on the battery frame). Hardened set: *xefe, Quecholli*. *Ixchel* is C+L: Battery E recovered the Cordemex herb string, not the approved theonym / Godstealer sense (Gate 1 miss; gloss unchanged). Split: H 2 / C\* 30 / C 45 / S 1. Rival-reading tournaments (`analysis/RIVAL_READING_TOURNAMENTS.md`) starred five more C\* rows (25 to 30). Selection-matched any-C Fisher is **p ≈ 0.095** trial (**p = 0.09494**) and **p ≈ 0.31** item (**p = 0.3069**); recovery-scored matched tests are **p = 1** / **p = 0.9991**. Syntax, gloss-blind analyst-level: 3/3 vs 0/2 recovered word-order regularities, Fisher p = 0.10, not significant; possession is not locked (`SYNTAX_EXPERIMENT_LOG_RERUN.md`). The 2026-07 3/3 vs 0/2 p = 0.10 is historical / not gloss-blind. Particle probes: `analysis/PARTICLE_PROBES_3_7.md` (leaning or still open; none newly settled). Dual-base tables are in §17.5-17.7.

**Line-level translation tests (not PPV).** Versus the 51 published English lines, methodology-on (T-M) and dictionaries-only (T-D) match+partial rates are both about 29% (T-M 75/255 = 29.4% [24.2, 35.3]; T-D 72/255 = 28.2% [23.1, 34.1]). Methodology does not lift vs-gold agreement (bootstrap difference CI includes zero); it cuts clash and raises abstain (`analysis/TRANSLATION_BATTERY_RESULTS.md`). Secure-line editorial grades on the same 51 lines: **0% secure** (0/51), about **24% working** (12/51 = 23.5% [14.0, 36.8]), about **75% fragile** (38/51 = 74.5% [61.1, 84.5]), 1/51 opaque-blocked (`analysis/SECURE_LINE_TAGS.md`). Leave-one-text-out: about **24% ok** (12/51 = 23.5% [14.0, 36.8]) and about **65% ok+partial** (33/51 = 64.7% [51.0, 76.4]); Text 3 is the weakest fold (0/13 ok) (`analysis/LEAVE_ONE_TEXT_OUT.md`). Model-based PPV and sentence-level percentages stay withdrawn as translation confidence. The honest unit of confidence is the individual token. L is a tag on H/C/S, never a lone tier. Companions: `NULL_MODEL_RESULTS_RERUN.md`, `SYNTAX_EXPERIMENT_LOG_RERUN.md`, `TRANSLATION_BATTERY_RESULTS.md`, `RIVAL_READING_TOURNAMENTS.md`, `PARTICLE_PROBES_3_7.md`, `LEAVE_ONE_TEXT_OUT.md`, `SECURE_LINE_TAGS.md`, `CONSISTENCY_CORRECTIONS_2026-09-16.md`.

**What this reconstruction is, and is not.** This is a fan reconstruction, careful in method but still speculative, not an authoritative or official account of the Vaal language. Two layers should be kept apart. **Transcriptions and corpus documentation are stable:** the five texts are on the page, source-validated where the game files exist, and the published English lines are frozen. **Interpretive and translation confidence is thin:** form-first lookup recovers the predeclared root, language class, and sense on **6 of 78** section-9 rows; **2 of 78** are H after Gate 2; matched null tests do not reject at 0.05, and recovery scoring removes the remaining apparent signal. Those tests do not establish that the developers intended any particular meaning, that a given gloss is the correct translation, or that the recovered grammar reflects a designed system rather than a pattern the method imposed. No living speakers exist and no design documents are public, so idiom, connotation, wordplay, and intended register are largely unrecoverable and are likely to remain so. Every reading here is offered as the best-supported hypothesis under a stated method. The committed lexicon is the project's chosen readings; 30 rows are C\* and 45 are C / C+L. The sentences are less certain than their words; the whole is a serious but provisional effort, not a settled decipherment.
...
**Confidence key (revised after the 2026-09-16 re-run, §17).** The tier is the protocol's certification of a recorded decode, not a direct measure of gloss correctness. Gate 1 asks whether the decode recovers *this row's* predeclared root, language class, and sense. Gate 2 asks whether a different-meaning competitor was written down. L is a tag, never a lone tier. Population: **78 rows**, matching `token_classification.csv`. Split on that sheet: **H 2 / C\* 30 / C 45 / S 1**.

- **H** hardened (top tier): Gate 1 recovery pass (strict latitude, matching the declared reading, no unexplained residue) AND Gate 2 survival (no strong different-meaning competitor, including same-language homophones). Protocol in `HARDENING_PROTOCOL.md`. Only two rows: *xefe* (H), *Quecholli* (H+L).
- **C** committed but latitude-dependent: an attested root under the looser latitude, a Gate 1 miss, or a former L-only row now tagged C+L with re-run gates recorded (mostly Gate 1 fail). *Ixchel* is C+L: Cordemex herb ≠ frozen Godstealer sense; gloss column unchanged.
- **S** soft / unresolved step. One section-9 row (*Xatlene*) is S+L; other softs live in §10.
- **L** lore-anchored, orthogonal: **H+L**, **C+L**, **S+L**, **C+L\***.
- A star (*) flags a logged different-meaning competitor (Gate 2 or the 2026-09-16 tournaments), not merely a cross-language one.

Current hardened set (2 of 78): *xefe, Quecholli*. Superscripts in the **Source** column link to §18; counts, the withdrawn 10^-15 headline, and the translation-battery / secure-line / LOTO figures are in §17.
...
Post T-M / T-D. Protocol, per-token brackets, and the line-9 *kuxte'* methodology example: `analysis/RIVAL_READING_TOURNAMENTS.md`. Table: `analysis/rival_tournament_brackets.csv`. Dictionaries via `dict_lookup.py`. Gold English named load-bearing tokens; it was not a search key. No new **H**. Losers are here, not deleted.
...
**All five texts are documented. Transcriptions and corpus notes are stable; interpretive and translation confidence is thin (§17).** Texts 1 and 2 are closed and source-validated against the game files; Texts 3, 4, and 5 are documented, with their unresolved tokens tracked in §10 rather than forced into the committed lexicon. The combined lexicon (§9) has **78 rows**, each with an H/C/S/O status (L is a tag). **2 are hardened** after the 2026-09-16 re-run and Ixchel Gate 1 honesty (*xefe*, *Quecholli*). *Ixchel* is C+L (herb ≠ approved theonym; gloss unchanged). Rival-reading tournaments (2026-09-16) moved five rows to C\* (*ik'el, 'Ibil, mucane, kifba, Tzokan'te*); *tlayeb* stays C+L with the ladder rival logged. Particle probes (§3.7) are leaning or still open (`analysis/PARTICLE_PROBES_3_7.md`). The full morpheme index (§15) inventories roughly 147 elements. Text 3, **Quemalani, the Elite Commander** (§6), contributes a five-line corpus with committed content tokens (folded into §9) and several flagged soft tokens (§10.4). Text 4, the **Drill Sergeant and Vaal Regiment** drill (§7), adds a fixed call-and-response built on committed roots (*Otsuks! ... U'te mucane!*), with its soft tokens (*a'tul*, *cheyel*, *Axba*, *quxzeh*, *Kíibsa'*) tracked in §10.9 and none promoted to §9.

**Statistical standing (full detail in §17).** The 2026-09-16 re-run of Batteries A-E, Gate 1, and Gate 2 is on disk (`NULL_MODEL_RESULTS_RERUN.md`, `gate1_rescore_rerun.csv`, `gate2_rescore_rerun.csv`). The 2026-08/09 audit rescore remains how the 2026-07 archives must be read. The 2 x 10^-15 / 3 x 10^-18 headlines stay withdrawn (`AUDIT_REVIEW_2026-08-18.md`). *pul / puul* now has a decode artifact (Gate 1 recovery yes, Gate 2 fall, C*). After recovery scoring and Ixchel Gate 1 honesty, **2 of 78 rows are hardened (2.6% [0.7, 8.9])** per token, **2 of 69 distinct roots (2.9% [0.8, 10.0])**; on the 61-row battery, 1/61 = 1.6% [0.3, 8.7] and 1/52 = 1.9% [0.3, 10.1]. Selection-matched any-C Fisher: **p ≈ 0.095** trial (**p = 0.09494**), **p ≈ 0.31** item (**p = 0.3069**); recovery-scored matched p = 1 / 0.9991. Model-based PPV and sentence-level percentages stay withdrawn as translation confidence. Translation batteries vs the 51 published English lines: match+partial about 29% on both arms (T-M 29.4% [24.2, 35.3]; T-D 28.2% [23.1, 34.1]); methodology does not lift agreement; it cuts clash and raises abstain (`analysis/TRANSLATION_BATTERY_RESULTS.md`). Secure-line grades on those 51 lines: 0/51 secure (0%), 12/51 working (~24%), 38/51 fragile (~75%), 1/51 opaque-blocked (`analysis/SECURE_LINE_TAGS.md`). Leave-one-text-out: ok 12/51 (~24%), ok+partial 33/51 (~65%), T3 0/13 ok (`analysis/LEAVE_ONE_TEXT_OUT.md`). Syntax panel (gloss-blind, analyst-level): 3/3 vs 0/2, Fisher p = 0.10, not significant; possession not locked (`SYNTAX_EXPERIMENT_LOG_RERUN.md`). Historical 2026-07 3/3 vs 0/2 p = 0.10 is not gloss-blind. Dual-base tables in §17.5-17.7. Tiers: **H** hardened, **C** committed, **C\*** competitor, **S** soft, **L** a tag only. Protocol: `HARDENING_PROTOCOL.md`. Ixchel honesty writeup: `analysis/CONSISTENCY_CORRECTIONS_2026-09-16.md`.
...
**Open items are not a short leftover list.** Thirty rows are C\* with logged competitors, and 45 remain C / C+L (latitude-dependent), including *Ixchel* after the Gate 1 sense miss. Named holdouts still tracked: the exact K'iche' lemma behind *jare'* (its apostrophe form now attested in Text 5); the unadopted *Ma'oxe* alternates; *fukuur*'s phonetic fit (*fulgor* adopted but soft); *ta'* (leaning relational *ti'*, not closed); and *ukto* in the Text 4 catechism, whose leading reading is now Nahuatl *ocotl* "pine torch" but stays soft (§10.7). The strongest original cross-language C\* case remains *itsok* vs Nahuatl *itztli* "obsidian". These are live co-readings, not settled glosses. *Guatelitzi* has left the open list (an attested in-game name, *Guatelitzi, Architect of Flesh* of the Temple of Atzoatl, now in §9, §10.8), and two former holdouts have rule-abiding leading candidates in §10.4: *buxa* ← *ba'ax* "what / what-thing" (keeps *x* = /ʃ/ intact) and *Eche lu* ← Spanish *échelo* "pour/cast it out" (supplies the *ch* /tʃ/ that *he'ela'* could not).

**Project status: texts and transcriptions stable; interpretive and translation confidence thin.** With all five known texts documented, the lexicon retiered (78 section-9 rows, 2 hardened after recovery scoring, Gate 2, and Ixchel Gate 1 honesty), and the syntax derived, the reconstruction has reached the limit of what the current corpus can support **on the texts**. The 10^-15 headline stays withdrawn. The 2026-07 batteries remain the historical record; the stopgap is how those files must be read; the re-run files are how current H/C counts, the syntax p, the T-M / T-D CIs, LOTO, and secure-line grades are read. Remaining analytical debt is more Vaal text. PPV / sentence bands stay withdrawn as translation confidence. New material would enlarge the sample, supply minimal pairs for open tokens, and test the §3 grammar against unseen clauses. The document is best read as stable on the texts, re-run-corrected on the lexicon batteries, modest on syntax significance (analyst-level p = 0.10, not significant; possession not locked), and thin on line-level translation agreement (match+partial about 29% both arms; 0/51 secure lines). The tooling (`HARDENING_PROTOCOL.md`, `score.py --rerun`, `run_gates_rerun.py`, `score_loto_secure.py`) is in place so new tokens can be scored without reopening the F1/N3 holes.
...
To test whether the committed readings are distinguishable from chance, and whether the published English is reproducible under this kit, the 2026-09-16 pass ran the lexicon null, both gates, Battery E, a gloss-blind syntax panel, T-M / T-D translation batteries, rival-reading tournaments, §3.7 particle probes, leave-one-text-out, and secure-line tags. Full protocol, seeds, and per-token data live beside this document. Current-epoch companions: `NULL_MODEL_RESULTS_RERUN.md`, `SYNTAX_EXPERIMENT_LOG_RERUN.md`, `TRANSLATION_BATTERY_RESULTS.md`, `RIVAL_READING_TOURNAMENTS.md`, `PARTICLE_PROBES_3_7.md`, `LEAVE_ONE_TEXT_OUT.md`, `SECURE_LINE_TAGS.md`, `CONSISTENCY_CORRECTIONS_2026-09-16.md`, `HARDENING_PROTOCOL.md`, and the run history in **`EXPERIMENT_LOG.md`**. Archive counterparts (`NULL_MODEL_RESULTS.md`, `ADVERSARIAL_RESULTS.md`, `SYNTAX_EXPERIMENT_LOG.md`) remain the 2026-07 record.

**Re-run versus stopgap.** The 2026-09-16 files (`NULL_MODEL_RESULTS_RERUN.md`, `gate1_rescore_rerun.csv`, `gate2_rescore_rerun.csv`, `BATTERY_E_RERUN.md`) are the current lexicon and null numbers in this section. The 2026-08/09 audit and recovery rescore (`AUDIT_REVIEW_2026-08-18.md`, `RESCORE_OUTPUT.md`, `NULL_HONESTY_OUTPUT.md`) remain the **STOPGAP** reading of the 2026-07 archives; those files were not overwritten. Syntax is the gloss-blind analyst-level panel in `SYNTAX_EXPERIMENT_LOG_RERUN.md` (3/3 vs 0/2, p = 0.10, possession not locked). The 2026-07 panel is historical / not gloss-blind. Dual-base tables (per token and per distinct root) stay on the page. Do not mix epochs.
...
- **Five batteries**, 5 seeds each (1729, 9001, 271828, 42, 55555): **A** loose latitude / offline dictionaries; **B** loose / extra Nahuatl-Nawat wordlists; **C** strict / offline; **D** strict / extra Nahuatl-Nawat (A-D each 500 pseudo + 150 real pooled, forming the latitude x dictionary-access 2x2, §17.3); **E** strict / extra-lexicon certification of three post-battery names as blind plants. The 2026-09-16 epoch used form-first lookup over attached text dictionaries; B/D did not browse the live web. 2026-07 A-D remain the original epoch.
...
|---|---|---|---|---|---|---|---|
| A loose / offline | 0.128 | 0.238 | 0.367 | 0.433 | +0.239 | 2.9x | 0.033 |
| B loose / + Nahuatl lists | 0.136 | 0.246 | 0.367 | 0.433 | +0.231 | 2.7x | 0.033 |
| C strict / offline | 0.064 | 0.182 | 0.233 | 0.333 | +0.169 | 3.6x | 0.033 |
| D strict / + Nahuatl lists | 0.068 | 0.186 | 0.233 | 0.333 | +0.165 | 3.4x | 0.033 |

The four batteries form a latitude x dictionary-access 2x2 (loose/strict against offline/extra-lexicon). Battery D (strict + Nahuatl/Nawat lists) used the same attached files as B; no live web (`NULL_MODEL_RESULTS_RERUN.md`, `blind_test_results_rerun_tight_online_s{seed}.csv`). Distinct-string count equals trial count (500/500).

Findings: (1) any-C discrimination is real but modest (about 16-24 points); recovery TPR is 3.3% in every cell, because exact hits often attach a different sense or language than `committed_readings.csv`. (2) Adding the Nahuatl extra lexicon did **not** widen the gap (D vs C: FPR 0.064 to 0.068, TPR flat). Latitude is still the axis that matters. (3) Tightening latitude (C vs A) cuts FPR (0.128 to 0.064) and any-C TPR (0.367 to 0.233) together. (4) The published claim that Vaal is **entirely random noise is rejected at p about 2 x 10^-15** stays **withdrawn as a headline**. **This epoch's selection-matched replacement** (`NULL_MODEL_RESULTS_RERUN.md`): any-C A-then-D trial p = **0.09494** (35/55 vs 32/64), item p = **0.3069** (7/11 vs 32/64); recovery-scored matched p = **1** / **0.9991**. The signal does not survive recovery scoring. No unconditional binomial versus a 4.6% floor. No sigma label. The stopgap matched range from the 2026-07 files (p about 0.005 to 0.06) is historical.

**Battery E (target certification, not a floor re-measurement).** Three names (*Quecholli*, *Panquetzaliztli*, *Ixchel*) were re-run as blind plants under Battery D conditions (`BATTERY_E_RERUN.md`). *Quecholli* 5/5 recovery (H+L); *Ixchel* 5/5 on the attested lexeme string (Cordemex herb gloss, not the approved theonym / Godstealer sense: Gate 1 miss, now C+L); *Panquetzaliztli* 0/5 (compound absent from attached dicts). Controls used post-rescore tiers: *naach* 5/5 recovery, *ek* 0/5 recovery / 5/5 any-C wasp (C, not H). Distractors 1/60 C. They do not restore an unconditional 10^-15 rejection. Counts on the 61-row and enlarged frames are in §17.7.

### 17.4 Adversarial survival and the hardened tier

Gate 2, same-language homophones on: **38 survive / 58 testable = 65.5%** on the 61-row frame, **50/75 = 66.7%** on the 78-row frame (`gate2_rescore_rerun.csv`). Residual limit: only competitors the lookup actually wrote can be scored. Some falls are OCR-junk alts in Cordemex; they still count because they were written down.

Combining recovery-scored Gate 1 with that Gate 2 and the hardening protocol, **2 of 78 section-9 rows are H** (1 of 61 battery-tested, plus *Quecholli*). *Ixchel* is C+L (Gate 1 sense miss). Thirty are C\* / C+L\* after the 2026-09-16 rival-reading tournaments (was 25; five new stars: *ik'el, 'Ibil, mucane, kifba, Tzokan'te*; lookup Gate-2 CSV not rewritten). Forty-five are C / C+L. *pul / puul* has a decode artifact (Gate 1 yes, Gate 2 fall, C\*). The hardened set is: *xefe, Quecholli*. Demoted from the stopgap H list: *ascensionada, che', -en, Eztli Pilli, ich, k'áak', ki', kujkuali, máax, náach, Ti, u, waaj, Panquetzaliztli, Ixchel*. Earlier demotions that remain demoted: *akal, ek, tul, uch', xi, k'ux, ma*. Tournament writeup: `analysis/RIVAL_READING_TOURNAMENTS.md`. Particle probes: `analysis/PARTICLE_PROBES_3_7.md`.

Going forward, no token may carry `H` without both gates recorded under recovery scoring. Proof of this epoch: `analysis/gate1_rescore_rerun.csv`, `analysis/gate2_rescore_rerun.csv`. Archive stopgap: `analysis/RESCORE_OUTPUT.md`.

### 17.5 Confidence interval for the translation

Full working, with Wilson 95% intervals and the base-rate sensitivity table, is in `STATISTICAL_SUMMARY.md`. The coverage figures below are from the 2026-09-16 re-run (`token_classification.csv`, `gate1_rescore_rerun.csv`, `gate2_rescore_rerun.csv`). Model-based PPV and sentence-level bands stay labelled **withdrawn as translation confidence**. They are not deleted.

**Inter-translator / vs-gold agreement is a different quantity.** Line-level English batteries T-M (methodology on) and T-D (dictionaries only), five seeds each, 51 surface lines, N = 255 line-slots per battery (`analysis/TRANSLATION_BATTERY_RESULTS.md`):
...
Bootstrap T-M minus T-D match+partial: +0.012, 95% percentile CI [-0.067, +0.090], includes zero. Methodology does not lift vs-gold line agreement on this corpus. It does change decoder behavior: T-M clash 139/255 = 54.5% [48.4, 60.5] vs T-D 172/255 = 67.5% [61.5, 72.9]; T-M abstain 41/255 = 16.1% [12.1, 21.1] vs T-D 11/255 = 4.3% [2.4, 7.6]. Those difference CIs exclude zero. Shared dictionaries still pull both arms into the same false friends when they assert (insect *ik'el*, shake *'Ibil*, bury *mucane*; logged in §10.13). Clash/abstain CIs and labeled exploratory tables (per-text, vocative sensitivity, Fleiss kappa) live in the same results file. Those rates are reproducibility under this dictionary kit, not PPV and not a sentence-band product. Do not convert them into the withdrawn PPV table below.

**Editorial stress test (secure-line grades).** For each of the same 51 published English lines, load-bearing tokens are tagged H / C / C* / S / O and the line is graded secure / working / fragile / opaque-blocked (`analysis/SECURE_LINE_TAGS.md`). Secure requires every load-bearing token to be H: **0/51 = 0.0% [0.0, 7.0]**. Working (all H or plain C): 12/51 = 23.5% [14.0, 36.8]. Fragile (any C* or S): 38/51 = 74.5% [61.1, 84.5]. Opaque-blocked (any O): 1/51 = 2.0% [0.3, 10.3] (Kamasan *jare'yantul*, because *yan* is not a §9 row). Almost no secure lines (none). About three quarters are fragile. This is the translation-confidence companion to the T-M / T-D CIs. It is not a PPV and not a sentence-band product. Working count is unchanged by the Ixchel demotion (the name is not in the 51 lines). The only H token in that corpus is *xefe* (line 35), and that line is still fragile because *te'moxti* is S.

**Leave-one-text-out.** Same 51 lines, five folds (`analysis/LEAVE_ONE_TEXT_OUT.md`). ok 12/51 = 23.5% [14.0, 36.8]; ok+partial 33/51 = 64.7% [51.0, 76.4]; fail 18/51 = 35.3% [23.6, 49.0]. Text 3 is the weakest fold: 0/13 ok. T4 ok is vocative-inflated (5/12; 0/7 if *Atziri!* and *U'te mucane* gifts drop). Macro-average ok 18.3%. This is internal transfer, not a new null battery.

**Measured coverage (assumption-free, re-run 2026-09-16, Ixchel honesty).** The hardened core of the committed lexicon is **2.6%, 95% CI [0.7%, 8.9%]** (2 of 78 section-9 tokens); counted over the 69 distinct roots it is **2.9%, 95% CI [0.8%, 10.0%]** (2 of 69). The 61-row battery gives **1.6% [0.3, 8.7]** and **1.9% [0.3, 10.1]** (1 of 61 / 1 of 52). The enlarged base, folding in the three Battery E names, gives **3.1% [0.9, 10.7]** and **3.6% [1.0, 12.3]** (2 of 64 / 2 of 55); see "A third base" in §17.7. Composition note: *Quecholli* is an exact attested lexeme string and so an easy plant relative to the text tokens; *Ixchel* hit the Cordemex string but not the approved sense (C+L); *Panquetzaliztli* missed because the compound is not in the attached dicts. The old 17/78 and 14/61 figures are the stopgap epoch, not the current certification rate. One caution: these figures are the *protocol's certification rate* (how often a reading survives hardening under form-first lookup), not a direct measure of translation correctness. The PPV below remains withdrawn as translation confidence.
...
The product-of-tokens figures above used the old any-C inputs and a lexicon that then had a large H set. After this re-run, **2 of 78** rows are H. Secure-line grades on the published English are 0/51 secure and 38/51 fragile. A typical line is not carried by hardened vocabulary. A line is no stronger than its least-secure token. Do not quote the withdrawn bands as current translation confidence.
...
**Mixture model, and why it is not usable here.** In principle b = (O - FPR) / (TPR - FPR) from the corpus's observed pass rate O. With the re-run numbers, recovery TPR (0.033) sits below D FPR (0.068), so the mixture is undefined or negative. Archive any-C TPR (0.140) is not the same diagnostic as the lexicon's protocol-clean Gate 1 rate (4/61). When O and TPR answer different questions the equation has no valid solution.
...
The counts above treat each committed token as an independent trial. That overstates the evidence wherever several tokens descend from one root: *ik'bala*, *ikba'yucane*, *Ik'eche*, and *ik'el* are all built on Yucatec *ik'* "spirit, breath," so they are four surface forms of a single lexical bet, not four independent confirmations. Counting them singly inflates both the sample size and the strict-pass tally. This subsection reruns the tests with morphological relatives collapsed to their shared root, so the result is visible on both bases. The recount below is stated on the **61-row battery** after the 2026-09-16 re-run; the enlarged headline base, with the three Battery-E names, is in "A third base" at the end of this subsection. *pul* now has a decode artifact and lives in the 78-row frame as C*, not in the 61-row battery.
...
| Root | Tokens merged | Strict (Gate 1), re-run recovery |
|---|---|---|
...
The 61 tokens thus reduce to **52 distinct roots** (the three Battery-E names each add a further distinct root, so the enlarged frame has **55**; the 78-row section-9 table, adding *pul* and the 13 former L-only rows as distinct heads, has **69**; see "A third base" below). None of these families holds an H token in this epoch. The only battery-frame H is *xefe*, which is not in a merge family, so the collapse removes no H (1 stays 1) while dropping nine mostly-failing forms from the denominator. Protocol-clean Gate 1 on this frame is 4 (*k'áak'*, *náach*, *tul*, *xefe*), all distinct heads, so 4 stays 4.
...
| Null-rejection p (selection-matched, any-C re-run) | trial 0.09494; item 0.3069 | unique strings already; same Fisher |
...
**Reading.** The dependency correction does not restore a large hardened core. Related tokens are concentrated in failing families, so the per-root H share only moves from 1.6% to 1.9% on the battery frame (2.6% to 2.9% on the 78-row table, 3.1% to 3.6% on the enlarged frame). The recount is defensive, not promotional. It does not restore a 10^-15 unconditional floor test. This epoch's matched any-C p is 0.09494 / 0.3069; recovery-scored matched p is 1 / 0.9991.
...
**Pseudo-corpus dependence (N1).** The 2026-09-16 noise arm contains **500 distinct strings of 500** (no cross-seed nonce repeats). The generator rejects corpus tokens and long substrings of corpus tokens (archived leak list: *eztl*, *noche*, *ko'ja*, ...). Ordinary Spanish exact hits on the loan palette are labelled Spanish C, not pure noise, and those strings were excluded from the pseudo arm. Distinct-string Battery D FPR is 34/500 = 6.8%. See `NULL_MODEL_RESULTS_RERUN.md`. The 2026-07 arm (426 distinct of 500) remains in `NULL_HONESTY_OUTPUT.md` as history.

**A third base: folding in the post-battery names.** Three onomastic tokens (*Quecholli*, *Panquetzaliztli*, *Ixchel*, §9, §12.5-12.6) were re-run as blind plants: **Battery E** (`BATTERY_E_RERUN.md`, §17.3), shuffled among new unique pseudos and post-rescore controls, under the strict-plus-extra-lexicon conditions of Battery D. *Quecholli* 5/5 (H+L), *Ixchel* 5/5 (lexeme string; Cordemex herb gloss ≠ approved theonym, now C+L), *Panquetzaliztli* 0/5 (not in attached dicts, C+L). The enlarged frame below is filled from those files. The 61-row / 78-row frames stay beside it. Unconditional binomials vs a 4.6% floor, and sigma labels, are not reused as headlines.
...
| Null-rejection p, per token | matched any-C p = 0.09494 / 0.3069; recovery-scored p = 1 / 0.9991 | E not mixed into the A-D Fisher | same A-D Fisher |
| Null-rejection p, per distinct root | A-D strings already unique | same | same |
| Hardened PPV band (data-anchored) | withdrawn as translation confidence | same | same |

(The enlarged distinct-root count assumes the three names add three new roots; if *quetza* in *Panquetzaliztli* is treated as a root already present in the corpus, the per-root H figure is still 2/54 because *Panquetzaliztli* is not H and *Ixchel* is not H.) Each name is counted as one observation by its head root rather than split into its constituent morphemes (*Panquetzaliztli* as a single *quetza*-headed bet; *Ixchel* as a single *chel*-headed bet). All three columns stay on the page: the 61-row battery is the recovery-scored A-D frame; the 64-row enlarged frame includes Battery E; the 78-row section-9 table is what a reader actually consults (all 78 gated in this epoch). *pul* now has an artifact (C*). Re-running when new material arrives remains the versioning trigger (§14).

### 17.8 Confirmation of the syntactic analysis (§3)

The grammar in §3 is the project's distributional reading. The current confirmation is the 2026-09-16 gloss-blind panel: analysts received numbered surface lines and a form-only token list (orthography, no project English glosses, no §3 conclusions). Scoring unit is analyst by corpus. Packets, raw sheets, and scoring: `SYNTAX_EXPERIMENT_LOG_RERUN.md`. Protocol: `SYNTAX_CONFIRMATION_PROTOCOL.md`, `EXPERIMENT_RERUN_PROTOCOL.md`.
...
**Current claim (gloss-blind, analyst-level).** Three real-panel analysts (A, B, C) versus two scrambled-panel analysts (D, E; within-line shuffle, seed 1729). An analyst recovered consistent word-order regularities if they reported, at medium or high confidence, a consistent linear order for at least two of questions 1-4. Result: **3/3 vs 0/2, Fisher one-sided p = 0.10**, not significant. N = 5. Small N; do not inflate it by counting correlated rules. Do not quote a rule-level table as the p-value.

Qualitative match to the six §3 conclusions is weaker without English than SYN-1. On the real panel, clause-initial type marking and prenominal *le* were partial for A and B and stronger for C; possessed-before-possessor was confirmed only by C (A and B undetermined); verb-object was partial or medium. Copula absence and affix-edge findings did not survive scramble as a clean positive in this panel.

**Historical 2026-07 panel (not gloss-blind).** Analysts received surface lines, English translations, and a token glossary. Analyst-level table from those files: **3/3 vs 0/2, Fisher p = 0.10**. Rule-level 12/12 vs 0/8 (p about 7.9 x 10^-6) is withdrawn as a significance claim (pseudoreplication). Full run: `SYNTAX_EXPERIMENT_LOG.md`. The same counts and p as the gloss-blind table do not make SYN-1 gloss-blind, and p = 0.10 is not significant either time.

**Honest caveat.** Shared corpus measures reproducibility of a reading from the data, not designer intent. Model-based PPV and sentence-level percentages stay withdrawn as translation confidence. Inter-translator and vs-gold line agreement CIs live in `analysis/TRANSLATION_BATTERY_RESULTS.md`; they are not a substitute for those withdrawn bands.
...
**Result.** The old equation "23/62 Gate 1 = strict-safe spellings" used the withdrawn any-C pass set. After this re-run the protocol-clean Gate 1 set is **4 of 61**. Permissive-rule specificity remains the null model's own measurement: the strict bar manufactures an any-C match 6.4% of the time offline (Battery C) or 6.8% with extra Nahuatl lists (Battery D), while the loose bar does so 12.8% of the time at C (Battery A; found-rate 43.3%). A derivation that leans on a permissive rule is close to noise as a spelling correspondence. Archive any-C rates (3.4% / 4.6% / 67%) stay in `NULL_MODEL_RESULTS.md`.
...
The single largest lever is the base rate b. The statistics constrain the test's error rates, but the translation's overall correctness still rides on how much of Vaal the designers built from real roots, which no internal test can measure. The 2026-09-16 re-run replaced any-C lexicon headlines with recovery-scored ones; it does not skip the need for b, so PPV stays withdrawn. Independently of b, the T-M / T-D match+partial rates (about 29% both arms) and the secure-line grades (0/51 secure, ~75% fragile) are the current measured statements about the published English.
