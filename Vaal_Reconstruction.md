...
**Inter-translator / vs-gold agreement is a different quantity.** Line-level English batteries T-M and T-D (five seeds each, 51 surface lines, N = 255 line-slots per battery) report Wilson 95% CIs on match and match+partial versus the published English, plus a bootstrap CI on the T-M minus T-D difference, in `analysis/TRANSLATION_BATTERY_RESULTS.md`. Those rates are reproducibility under this dictionary kit, not PPV and not a sentence-band product. Methodology did not raise match+partial versus gold on this corpus (difference CI includes zero). Do not convert them into the withdrawn PPV table below.
