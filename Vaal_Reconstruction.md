...
**Statistical standing (full detail in §17).** The reconstruction has been stress-tested against a null model of phonotactically Vaal-like nonsense. The 2026-09-16 re-run under `analysis/EXPERIMENT_RERUN_PROTOCOL.md` is now on disk (recovery Gate 1, same-language Gate 2, de-duplicated null). The 2026-08/09 audit rescore remains the **historical STOPGAP** for the 2026-07 files. The published all-noise rejection of about 2 x 10^-15 (and the enlarged-base 3 x 10^-18), including the sigma column, is **withdrawn as a headline**. After the re-run, **3 of 78 section-9 rows are hardened (3.8%, 95% CI 1.3-10.7%)** per token, or **4.3% [1.5, 12.0]** counted over 69 distinct roots (1 of 61 = 1.6% [0.3, 8.7] / 1 of 52 = 1.9% [0.3, 10.1] on the battery frame). Hardened set: *xefe, Quecholli, Ixchel*. Rival-reading tournaments (2026-09-16, `analysis/RIVAL_READING_TOURNAMENTS.md`) starred five more C\* rows (25 to 30); H is unchanged. Selection-matched any-C Fisher is **p = 0.09494** (trial) and **p = 0.3069** (item); recovery-scored matched tests are **p = 1** / **p = 0.9991**. Syntax, gloss-blind analyst-level: 3/3 vs 0/2 recovered word-order regularities, Fisher p = 0.10, not significant (`SYNTAX_EXPERIMENT_LOG_RERUN.md`). The 2026-07 3/3 vs 0/2 p = 0.10 is historical / not gloss-blind. Particle probes: `analysis/PARTICLE_PROBES_3_7.md` (leaning or still open; none newly settled). Model-based PPV and sentence-level percentages stay withdrawn as translation confidence. The honest unit of confidence is the individual token. L is a tag on H/C/S, never a lone tier. Dual-base tables are in §17.5-17.7.
...
**Confidence key (revised after the 2026-09-16 re-run, §17).** The tier reflects whether a recorded decode recovers *this row's* predeclared root, language class, and sense, then survives Gate 2. L is a tag, never a lone tier. Population: **78 rows**, matching `token_classification.csv`.
...
Current hardened set (3 of 78): *xefe, Quecholli, Ixchel*. Superscripts in the **Source** column link to §18; counts and the withdrawn 10^-15 headline are in §17.
...
`gate2_rescore_rerun.csv` is not rewritten. `token_classification.csv` records the tournament falls. Section-9 C\* count 25 to 30. H stays 3.
...
**All five texts are documented, and the reconstruction now rests on a measured statistical footing (§17).** Texts 1 and 2 are closed and source-validated against the game files; Texts 3, 4, and 5 are documented, with their unresolved tokens tracked in §10 rather than forced into the committed lexicon. The combined lexicon (§9) has **78 rows**, each with an H/C/S/O status (L is a tag). 3 are hardened after the 2026-09-16 re-run. Rival-reading tournaments (2026-09-16) moved five rows to C\* (*ik'el, 'Ibil, mucane, kifba, Tzokan'te*); H is unchanged. Particle probes (§3.7) are leaning or still open (`analysis/PARTICLE_PROBES_3_7.md`). The full morpheme index (§15) inventories roughly 147 elements. Text 3, **Quemalani, the Elite Commander** (§6), contributes a five-line corpus with committed content tokens (folded into §9) and several flagged soft tokens (§10.4). Text 4, the **Drill Sergeant and Vaal Regiment** drill (§7), adds a fixed call-and-response built on committed roots (*Otsuks! ... U'te mucane!*), with its soft tokens (*a'tul*, *cheyel*, *Axba*, *quxzeh*, *Kíibsa'*) tracked in §10.9 and none promoted to §9.

**Statistical standing (full detail in §17).** The 2026-09-16 re-run of Batteries A-E, Gate 1, and Gate 2 is on disk (`NULL_MODEL_RESULTS_RERUN.md`, `gate1_rescore_rerun.csv`, `gate2_rescore_rerun.csv`). The 2026-08/09 audit rescore remains how the 2026-07 archives must be read. The 2 x 10^-15 / 3 x 10^-18 headlines stay withdrawn (`AUDIT_REVIEW_2026-08-18.md`). *pul / puul* now has a decode artifact (Gate 1 recovery yes, Gate 2 fall, C*). After recovery scoring, **3 of 78 rows are hardened (3.8% [1.3, 10.7])** per token, **3 of 69 distinct roots (4.3% [1.5, 12.0])**; on the 61-row battery, 1/61 = 1.6% [0.3, 8.7] and 1/52 = 1.9% [0.3, 10.1]. Selection-matched any-C Fisher: **p = 0.09494** trial, **p = 0.3069** item; recovery-scored matched p = 1 / 0.9991. Model-based PPV and sentence-level percentages stay withdrawn as translation confidence. Secure-line grades on the 51 published English lines: 0/51 secure, 12/51 working, 38/51 fragile, 1/51 opaque-blocked (`analysis/SECURE_LINE_TAGS.md`). Leave-one-text-out: ok 12/51, T3 0/13 ok (`analysis/LEAVE_ONE_TEXT_OUT.md`). Syntax panel (gloss-blind, analyst-level): 3/3 vs 0/2, Fisher p = 0.10, not significant (`SYNTAX_EXPERIMENT_LOG_RERUN.md`). Historical 2026-07 3/3 vs 0/2 p = 0.10 is not gloss-blind. Dual-base tables in §17.5-17.7. Tiers: **H** hardened, **C** committed, **C\*** competitor, **S** soft, **L** a tag only. Protocol: `HARDENING_PROTOCOL.md`. Proof: `analysis/NULL_MODEL_RESULTS_RERUN.md`, `analysis/gate1_rescore_rerun.csv`.
...
**Open items are narrow, and now include the logged adversarial competitors.** Still open: the exact K'iche' lemma behind *jare'* (its apostrophe form now attested in Text 5); the unadopted *Ma'oxe* alternates; *fukuur*'s phonetic fit (*fulgor* adopted but soft); *ta'* (leaning relational *ti'*, not closed); and *ukto* in the Text 4 catechism, whose leading reading is now Nahuatl *ocotl* "pine torch" but stays soft (§10.7). Thirty tokens carry a logged different-meaning competitor (**C\***, including same-language homophones); the strongest original cross-language case remains *itsok* vs Nahuatl *itztli* "obsidian". These are live co-readings, not settled glosses. *Guatelitzi* has left the open list (an attested in-game name, *Guatelitzi, Architect of Flesh* of the Temple of Atzoatl, now in §9, §10.8), and two former holdouts have rule-abiding leading candidates in §10.4: *buxa* ← *ba'ax* "what / what-thing" (keeps *x* = /ʃ/ intact) and *Eche lu* ← Spanish *échelo* "pour/cast it out" (supplies the *ch* /tʃ/ that *he'ela'* could not).

**Project status: texts stable; inferential layer is the 2026-09-16 re-run, including the gloss-blind syntax panel.** With all five known texts documented, the lexicon retiered (78 section-9 rows, 3 hardened after recovery scoring and Gate 2), and the syntax derived, the reconstruction has reached the limit of what the current corpus can support **on the texts**. The 10^-15 headline stays withdrawn. The 2026-07 batteries remain the historical record; the stopgap is how those files must be read; the re-run files are how current H/C counts and the syntax p are read. Remaining analytical debt is more Vaal text, and PPV / sentence bands stay withdrawn as translation confidence. New material would enlarge the sample, supply minimal pairs for open tokens, and test the §3 grammar against unseen clauses. The document is best read as stable on the texts, re-run-corrected on the lexicon batteries, and modest on syntax significance (analyst-level p = 0.10, not significant). The tooling (`HARDENING_PROTOCOL.md`, `score.py --rerun`, `run_gates_rerun.py`) is in place so new tokens can be scored without reopening the F1/N3 holes.
...
Totals: 147 elements. By the best tier each element reaches (stopgap recount, 2026-09): 22 Hardened, 53 Committed, 12 Committed (Competitor), 23 Soft, 37 Lore. The 2026-09-16 re-run retiered the §9 rows (3 H of 78: *xefe*, *Quecholli*, *Ixchel*); a full per-element recount of this index is pending. Battery-E morphemes: *quechōl-*, *ix-*, and *chel* still head H+L names; *pan(tli)*, *quetza*, and *-liztli* now head *Panquetzaliztli* at C+L (§12.5-12.6).
...
**Re-run versus stopgap.** The 2026-09-16 files (`NULL_MODEL_RESULTS_RERUN.md`, `gate1_rescore_rerun.csv`, `gate2_rescore_rerun.csv`, `BATTERY_E_RERUN.md`) are the current lexicon and null numbers in this section. The 2026-08/09 audit and recovery rescore (`AUDIT_REVIEW_2026-08-18.md`, `RESCORE_OUTPUT.md`, `NULL_HONESTY_OUTPUT.md`) remain the **STOPGAP** reading of the 2026-07 archives; those files were not overwritten. Syntax is the gloss-blind analyst-level panel in `SYNTAX_EXPERIMENT_LOG_RERUN.md` (3/3 vs 0/2, p = 0.10). The 2026-07 panel is historical / not gloss-blind. Dual-base tables (per token and per distinct root) stay on the page. Do not mix epochs.
...
**Battery E (target certification, not a floor re-measurement).** Three names (*Quecholli*, *Panquetzaliztli*, *Ixchel*) were re-run as blind plants under Battery D conditions (`BATTERY_E_RERUN.md`). *Quecholli* 5/5 recovery; *Ixchel* 5/5 on the attested lexeme string (Cordemex herb gloss, not a goddess entry); *Panquetzaliztli* 0/5 (compound absent from attached dicts). Controls used post-rescore tiers: *naach* 5/5 recovery, *ek* 0/5 recovery / 5/5 any-C wasp (C, not H). Distractors 1/60 C. *Quecholli* and *Ixchel* remain H+L; *Panquetzaliztli* is C+L. They do not restore an unconditional 10^-15 rejection. Counts on the 61-row and enlarged frames are in §17.7.
...
Combining recovery-scored Gate 1 with that Gate 2 and the hardening protocol, **3 of 78 section-9 rows are H** (1 of 61 battery-tested, plus *Quecholli* and *Ixchel*). Thirty are C\* / C+L\* after the 2026-09-16 rival-reading tournaments (was 25; five new stars: *ik'el, 'Ibil, mucane, kifba, Tzokan'te*; lookup Gate-2 CSV not rewritten). *pul / puul* has a decode artifact (Gate 1 yes, Gate 2 fall, C\*). The hardened set is: *xefe, Quecholli, Ixchel*. Demoted from the stopgap H list: *ascensionada, che', -en, Eztli Pilli, ich, k'áak', ki', kujkuali, máax, náach, Ti, u, waaj, Panquetzaliztli*. Earlier demotions that remain demoted: *akal, ek, tul, uch', xi, k'ux, ma*. Tournament writeup: `analysis/RIVAL_READING_TOURNAMENTS.md`. Particle probes: `analysis/PARTICLE_PROBES_3_7.md`.
...
**Inter-translator / vs-gold agreement is a different quantity.** Line-level English batteries T-M and T-D (five seeds each, 51 surface lines, N = 255 line-slots per battery) report Wilson 95% CIs on match and match+partial versus the published English, plus a bootstrap CI on the T-M minus T-D difference, in `analysis/TRANSLATION_BATTERY_RESULTS.md`. Clash and abstain CIs, and labeled exploratory tables (per-text, hotspots, Fleiss kappa), live in the same file. Those rates are reproducibility under this dictionary kit, not PPV and not a sentence-band product. Methodology did not raise match+partial versus gold on this corpus (difference CI includes zero). It did raise abstain and cut clash (those difference CIs exclude zero). Do not convert them into the withdrawn PPV table below.

**Editorial stress test (secure-line grades).** For each of the same 51 published English lines, load-bearing tokens are tagged H / C / C* / S / O and the line is graded secure / working / fragile / opaque-blocked (`analysis/SECURE_LINE_TAGS.md`). Secure requires every load-bearing token to be H: **0/51 = 0.0% [0.0, 7.0]**. Working (all H or plain C): 12/51 = 23.5% [14.0, 36.8]. Fragile (any C* or S): 38/51 = 74.5% [61.1, 84.5]. Opaque-blocked (any O): 1/51 = 2.0% [0.3, 10.3] (Kamasan *jare'yantul*, because *yan* is not a §9 row). This is the translation-confidence companion to the T-M / T-D CIs. It is not a PPV and not a sentence-band product. Leave-one-text-out transfer of the same 51 lines: `analysis/LEAVE_ONE_TEXT_OUT.md` (ok 12/51 = 23.5% [14.0, 36.8]; T3 0/13 ok).

**Measured coverage (assumption-free, re-run 2026-09-16).** The hardened core of the committed lexicon is **3.8%, 95% CI [1.3%, 10.7%]** (3 of 78 section-9 tokens); counted over the 69 distinct roots it is **4.3%, 95% CI [1.5%, 12.0%]** (3 of 69). The 61-row battery gives **1.6% [0.3, 8.7]** and **1.9% [0.3, 10.1]** (1 of 61 / 1 of 52). The enlarged base, folding in the three Battery E names, gives **4.7% [1.6, 12.9]** and **5.5% [1.9, 14.9]** (3 of 64 / 3 of 55); see "A third base" in §17.7. Composition note: two of the three Battery-E names are exact attested lexeme strings and so are easy plants relative to the text tokens; *Panquetzaliztli* missed because the compound is not in the attached dicts. The old 17/78 and 14/61 figures are the stopgap epoch, not the current certification rate. One caution: these figures are the *protocol's certification rate* (how often a reading survives hardening under form-first lookup), not a direct measure of translation correctness. The PPV below remains withdrawn as translation confidence.
...
**Reading.** The dependency correction does not restore a large hardened core. Related tokens are concentrated in failing families, so the per-root H share only moves from 1.6% to 1.9% on the battery frame (3.8% to 4.3% on the 78-row table, 4.7% to 5.5% on the enlarged frame). The recount is defensive, not promotional. It does not restore a 10^-15 unconditional floor test. This epoch's matched any-C p is 0.09494 / 0.3069; recovery-scored matched p is 1 / 0.9991.
...
**A third base: folding in the post-battery names.** Three onomastic tokens (*Quecholli*, *Panquetzaliztli*, *Ixchel*, §9, §12.5-12.6) were re-run as blind plants: **Battery E** (`BATTERY_E_RERUN.md`, §17.3), shuffled among new unique pseudos and post-rescore controls, under the strict-plus-extra-lexicon conditions of Battery D. *Quecholli* 5/5, *Ixchel* 5/5 (lexeme string; Cordemex herb gloss), *Panquetzaliztli* 0/5 (not in attached dicts). *Quecholli* and *Ixchel* remain H+L; *Panquetzaliztli* is C+L. The enlarged frame below is filled from those files. The 61-row / 78-row frames stay beside it. Unconditional binomials vs a 4.6% floor, and sigma labels, are not reused as headlines.
...
|---|---|---|---|
| Hardened tier, per token | 1.6% [0.3, 8.7] (1/61) | 4.7% [1.6, 12.9] (3/64) | 3.8% [1.3, 10.7] (3/78) |
| Hardened tier, per distinct root | 1.9% [0.3, 10.1] (1/52) | 5.5% [1.9, 14.9] (3/55) | 4.3% [1.5, 12.0] (3/69) |
| Strict Gate-1 protocol-clean, per token | 4 | 6 (*xefe*, *k'áak'*, *náach*, *tul*, *Quecholli*, *Ixchel*; *pul* is id 62) | 7 (adds *pul*) |
...
The single largest lever is the base rate b. The statistics constrain the test's error rates tightly, but the translation's overall correctness still rides on how much of Vaal the designers built from real roots, which no internal test can measure. The 2026-09-16 re-run replaced any-C lexicon headlines with recovery-scored ones; it does not skip the need for b, so PPV stays withdrawn.
