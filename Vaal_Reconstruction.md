...
**Headline.** Seven committed words have a genuine other-language rival. None of those rivals replaced the committed reading. Each is logged here and starred in §9.

Question: if a Vaal word is looked up from its spelling alone, and the search is told to prefer a root from a *different* source language than the one already chosen, does that search find a real other meaning? What was done: a decoder who could not see the existing readings was given only the surface form and hunted the best-attested root in another palette language. Why it matters: a reading forced by spelling alone would have no such rival. A logged rival means the committed English is a choice among attested options, not the only option the letters allow. No share with a confidence interval is claimed here; the live counts for "no strong rival" are the Gate 2 figures in §17.4.
...
- **mucane** Maya *muk'* "strength" vs K'iche' *muq* "bury" (already carried as the shadow sense). The later contest in §10.13 made Yucatec *muk* "sepultar" the same-language bury hit and the starred co-reading. The row is **C\***.

Four other apparent rivals (*ma*, *qexcan*, *sakilja*, *Yutsal*) were set aside as sister-language cognates: the "competing" K'iche' or Maya form is the *same* root in a related language (for example Maya *sak* = K'iche' *saq* "white"), which is corroboration, not competition. Full method and the survival tally are in `ADVERSARIAL_RESULTS.md`; the summary is in §17.

Same-language false friends from the later independent translations (insect *ik'el*, shake *'Ibil*, bury *mucane*, wax *kifba*, tear-out *Tzokan'te*, and the cross-graft ladder on *tlayeb*) are the log in §10.13; they are not deleted.

### 10.13 Rival-reading tournaments (2026-09-16)

**Headline.** Independent translations often landed on the same dictionary false friends. Those rivals were scored against the committed readings, using neighboring words and other lines as the field, not the published English as a search key. Five new stars. No new hardened word. No committed gloss was rewritten.

Question: when two independent translation kits keep hitting the same other dictionary sense for a word, is that other sense actually better, or does the committed reading still win on form and field? What was done: for each such word, two to four attested roots were frozen and scored on form fit, leftover letters, sense across every line the word appears in, morphology with related words, and neighboring tokens. Why it matters: a silent win for the committed reading would hide a real homophone. A silent rewrite would change a published meaning without a protocol pass. Losers stay here. Winners that did not move the gloss stay on the same lemma.

Protocol, per-token brackets, and the line-9 *kuxte'* example: `analysis/RIVAL_READING_TOURNAMENTS.md`. Table: `analysis/rival_tournament_brackets.csv`. Dictionaries via `dict_lookup.py`. The published English named which words carried the line; it was not a search key.

**Winners that did not move the gloss (same lemma or field win):**

- *kux / kuxkal / kuxte'*: Yucatec *kux/kuxtal* "life" beats Nawat *kuxta* "cacao-like tree." All five method-notes translations of that line recovered life and death; all five dictionaries-only translations recovered cacao-tree and death. The win is methodology plus merism plus the *kuxkal* family. Stays **C+L\***.
- *kutsen*: the "turkey" hit is Cordemex *KUTS* "pavo," the same lemma as "sacrificial bird." The lore step "offering" stays. Brush-clean (*KUTS* acepillar) and Nawat *-kuts* "leg" lose the ritual field. Stays **C+L\***.
- *tlayeb*: no whole-form hit. Ladder is Cordemex *YEB* plus Nah. *tla-*, a cross-graft, the dictionaries-only favorite; the method-notes arm abstained. Night/dark kept. Stays **C+L**. Category still open, §3.7.
- *i'chian*: Nahuatl 1100 *ichan* "his/her house" is exact. Clash was coverage. Stays **C**.

**New stars (exact or equal competitors the rival-meaning lookup did not write):**
...
- *'Ibil* **C+L\***: Cordemex *IBIL* "sacudir" (exact) and *EBIL* "escalera." Lost on the presentative couplet with *ik'el*. Stronger than lima bean *ib*. Flesh remains a lore step.
...
**Logged and not adopted (exact dictionary hits that lost the field):** insect *ik'el*; shake/ladder *'Ibil*; bury *mucane*; turkey-as-only-sense and brush/leg *kutsen*; cacao-tree *kuxta*; ladder/arrange *tlayeb*; *KIL* leftover on *kilya*; *KIB* wax on *kifba*; *yuq* stretch on *yuquia*; *TSOK* tear-out on *Tzokan'te*; *muj* specter leftover on *mujuk'*; excrement *ta'* (particle probe).

`gate2_rescore_rerun.csv` is not rewritten. `token_classification.csv` records the contest outcomes. The lexicon's starred count moved from 25 to 30. After these contests the hardened count was briefly 3; the same day's *Ixchel* lookup-honesty pass left it at 2 of 78.

### 10.14 Morphological residue and grouping footnotes (2026-09-16)

**Headline.** Related words are not independent proofs. The distinct-root counts stay 52 / 55 / 69. Shared pieces that are not both words' heads are footnotes, not merges.

Question: if two lexicon rows share a piece of a word, have we been counting one lexical bet twice? What was done: rows were merged only when they share the same head root. Shared affixes and shared non-head pieces were listed as footnotes. Why it matters: merging on a shared piece would be a new etymology of the second head. Leaving a shared piece un-noted would treat it as two independent confirmations.

Family audit: `analysis/MORPHOLOGICAL_FAMILY_AUDIT.md`. Prefer deleting duplicate independent-count claims over new etymologies. Shared pieces that are not both rows' heads are footnotes, not merges: *ts'ook* in *cha'tsoke* and *Tzokan'te*; *ma'* in *ma* and *Ma'oxe*; tails *-ba'* and *-ane*. *tul* "decline" and *pul* "throw" stay two roots. *kux* life and *k'ux* bite stay two roots. No *muk'zeh* class; *quxzeh* keeps unexplained *-zeh*.
...
**Current standing (plain headlines; numbers in §17).** Only two lexicon rows survive both strict checks. Pessimistic: 0.7% (lower 95% bound). Average: **2 of 78 = 2.6%** (*xefe*, *Quecholli*). Optimistic: 8.9% (upper 95% bound). By distinct root: 2 of 69 = 2.9% [0.8, 10.0]. On the smaller chance-check list: 1 of 61 = 1.6% [0.3, 8.7] / 1 of 52 roots = 1.9% [0.3, 10.1]. Chance tests: this is a yes/no result, not a range; even the more generous any-hit reading fails to reject chance at 0.05 (p = 0.09494 trial / 0.3069 item), and the stricter declared-sense reading removes the remaining signal (p = 1 / 0.9991). Line-level English, same three readings per metric, both arms (method-notes / dictionaries-only): exact match pessimistic 7.4% / 5.8%, average 10.6% / 8.6%, optimistic 15.0% / 12.7%; match or partial pessimistic 24.2% / 23.1%, average 29.4% / 28.2% (about 29%), optimistic 35.3% / 34.1%; secure pessimistic and average **0/51 = 0.0%**, optimistic 7.0%; working pessimistic 14.0%, average 23.5%, optimistic 36.8%. Leave-one-text-out: ok pessimistic 14.0%, average 12/51 = 23.5%, optimistic 36.8%; ok or partial pessimistic 51.0%, average 33/51 = 64.7%, optimistic 76.4%; Text 3 is the weakest (0/13 ok). Grammar: a single small-sample test, 3/3 vs 0/2, Fisher p = 0.10, not significant; possession is not locked. Tiers: **H** hardened, **C** committed, **C\*** rival logged, **S** soft, **L** a tag only. Historical / withdrawn displays: §17.10.
...
**Project status.** With all five known texts documented, the lexicon retiered (78 rows, 2 hardened), and the syntax derived, the reconstruction has reached the limit of what the current corpus can support **on the texts**. Remaining analytical debt is more Vaal text. New material would enlarge the sample, supply minimal pairs for open tokens, and test the §3 grammar against unseen clauses. The document is best read as stable on the texts, thin on certified vocabulary, modest on grammar significance (p = 0.10, not significant; possession not locked), and thin on line-level English agreement (match or partial about 29% both arms; 0/51 secure lines). Scoring tools under `analysis/` are in place so new tokens can be tested the same way.
...
Live results only in §17.1-17.9. Past-iteration artifacts are in §17.10 and are not current claims. Numbers below are from the 2026-09-16 experimental pass. Dual-base tables (per token and per distinct root) are in §17.7.
...
A 95% CI is the range that would contain the true share in 95% of repeats of this counting procedure. Where a result is a range, this section labels three readings **for that same metric**: pessimistic (lower 95% bound, or a stricter counting rule), average (point estimate, without restating the interval), optimistic (upper 95% bound, or a looser counting rule). When two translation arms exist, both are shown at each reading. No numbers are invented outside the live record. Where a check is a single yes/no test, that is stated instead of faking three shares.
...
Question: does the lookup certify nonsense strings that only look like Vaal as often as it certifies real corpus words? What was done: real committed tokens were mixed with fake Vaal-like strings, labels hidden, and looked up from spelling alone. A dictionary hit on a fake string is a false alarm by construction. Why it matters: if fakes pass as often as real words, a dictionary hit is not evidence. If real words pass more often, there is some signal above chance.
...
Question: if a Vaal word is looked up from its spelling alone, does that lookup return this project's declared dictionary root, language, and sense, and is there no strong rival meaning beside it? What was done: all 78 lexicon entries were looked up that way, without using the desired English as a search key, then checked for rival meanings (including same-language homophones). Why it matters: a pass would mean that reading is hard to get by accident. A fail means the reading remains a chosen hypothesis, not a certified one.
...
Question: if someone translates the Vaal lines from dictionaries (with or without this project's method notes), how often do they match the published English? What was done: independent translations of the 51 published lines were scored against that frozen English. One arm used this project's method notes; the other used dictionaries only. Why it matters: high agreement would mean the English on the page is reproducible from the Vaal with this kit. Low agreement means those translations are not strongly recovered.

**How the grades are named.** A **match** is an independent translation that makes the same claim about the same people or events as the published English (wording may differ). A **partial** is the same claim with different wording, or a small mix-up of who did what. **Match or partial** (match+partial) adds those two grades together. A **clash** is a different claim. An **abstain** is no English claim offered. The method-notes arm is **T-M**; the dictionaries-only arm is **T-D**. Three readings below are the lower 95% bound, the point estimate, and the upper 95% bound of **that same rate**. Average does not restate the interval. Both arms are shown.
...
Question: of the named figures in §12, how many carry an attested Mesoamerican (or Spanish) dictionary root? What was done: the closed name roster was counted, setting aside four borrowings from outside that palette. Why it matters: it is a descriptive fact about naming, not a probability that a text gloss is right. Converting it into a gloss-probability is not a live result (see §17.10).

**Proper-noun proxy (palette-scoped).** The named roster in §12 is a closed sample of the developers' naming behaviour. Four of its figures are documented borrowings from *outside* the Mesoamerican palette entirely: Apep (Egyptian), Ralakesh (Sanskrit), Arakaali (Greek/Hindu), and the Omnitect (Latin). These were never attempts at a palette root and are out of scope for this estimate. Among the **33 in-scope names, 19 carry an attested palette root (Nahuatl, Maya, K'iche', or Spanish): about 58%.** Many rest on structural markers hard to produce by chance, the Nahuatl *-tl / -tli / -atl* absolutive endings on *Matatl*, *Cholotl*, *Citaqualotl* and the like.

**Bounds on that figure.** Pessimistic: counting only clean, fully-worked parses (setting soft candidates aside) gives a floor of about 15-18%. Average: 19 of 33 = about 58% (soft-inclusive; already the looser count). Optimistic: the upper 95% bound on that same 19/33 count is 72.8%; the roster is not stretched to 75%. Folding the four out-of-scope borrowings back into the denominator is 19/37 about 51%, but that penalizes the estimate for deliberate design choices the palette was never meant to catch, so it is not used as any of the three readings.
...
Question: if several Vaal words share one dictionary root, have we been counting one lexical bet as four independent confirmations? What was done: morphological relatives were collapsed to their shared head root, and every coverage table was restated per token and per distinct root. Why it matters: a large hardened core that vanished under this recount would have been an illusion of repeated counting. A core that stays small was already small.
...
Question: do the word-order regularities in §3 show up to readers who are not given this project's English, and more often than when the same lines are scrambled? What was done: analysts received numbered Vaal lines and a form-only word list (no project English, no §3 conclusions). A control group received the same lines with the words shuffled inside each line. Why it matters: a clear, significant gap would support the grammar as something in the Vaal itself. No significant gap means the grammar is still a reading of a small corpus.
...
Question: which spelling-to-source mappings are tight, and which add or rearrange letters the source language does not have? What was done: the chance check's strict-versus-loose spelling rules were read as a test of those correspondences (no new audio experiment). Why it matters: a permissive spelling trick that also matches nonsense strings is close to noise as evidence. A strict-safe mapping is licensed by the source itself.
...
**All-noise headlines (withdrawn; not current).** Question those writeups asked: is the committed lexicon distinguishable from pure noise? What they did: counted any dictionary hit as a pass and compared that count to a fixed noise floor. Why a reader might have cared: a tiny p-value would have meant Vaal is not random letter-salad. Why it is not canon: the comparison treated any dictionary hit as a pass and used an invalid noise floor. A 2026-07 writeup claimed p about 2 x 10^-15 (enlarged-base 3 x 10^-18), including a sigma column and "probable lexicon" / "very unlikely pure noise" wording. Unconditional binomials versus a 4.6% floor are not reused.

**2026-07 files and the 2026-08/09 rereading (historical).** Question: how should the original chance-check worksheets be counted? What was done: the archives were reread without overwriting them. That rereading is not this pass's live result. An any-hit matched range from those older files (p about 0.005 to 0.06) is not mixed into §17.1-17.9.

**Earlier hardened counts (withdrawn as current certification).** Question those writeups asked: how many lexicon rows survive the strictest checks? What they did: counted any-dictionary-hit passes. Counts of 17 of 78, 14 of 61, 22 of 62, or 23 of 62 used that looser rule, not declared-sense recovery. The live set is *xefe* and *Quecholli* (2 of 78).

**Per-reading PPV tables (withdrawn).** Question those tables asked: given that a word was certified, what is the probability it is a genuine root rather than a chance dictionary hit? What they did: converted chance-check rates into a gloss-probability using a guessed fraction of Vaal built from real roots. They mixed different tests on different tiers (so committed could score higher than hardened at the same prior) and used any-hit rates rather than declared-sense recovery. They are not a probability that the English gloss is right. They are not reprinted here.

**Per-line confidence bands (withdrawn).** Question those bands asked: how sure is a whole English line? What they did: multiplied the withdrawn PPV values across the words in a line, including an independence product that is not a valid lower bound. Concrete percentages quoted for *Tlayeb kifba!*, *Atziri, Atziri, ascenada akal!*, and *Ti ek tala jare'yantul!* belonged to that method. They are not current results. Do not treat them as per-line confidence.

**2026-07 grammar panel (not the current claim).** Question that panel asked: can analysts recover the §3 word-order regularities? What they did: analysts received surface lines, English translations, and a token glossary, so they were not blind to this project's English. The analyst-level table was also 3/3 vs 0/2, Fisher p = 0.10. A rule-level 12/12 vs 0/8 (p about 7.9 x 10^-6) is withdrawn as a significance claim (the rules are not independent trials). Same counts as the later table do not make that panel blind to English. The live grammar claim is the 2026-09-16 panel in §17.8.
